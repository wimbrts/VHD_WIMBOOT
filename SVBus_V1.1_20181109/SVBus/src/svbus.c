...
NTSTATUS BusAddChild(IN PDEVICE_OBJECT BusDeviceObject,IN ULONG i)
{
	...
	// for HDD and CDROM we return only a mode page header, no other mode pages are necessary
	else if(DeviceExtension->Disk.VirtualDeviceType == HDD || DeviceExtension->Disk.VirtualDeviceType == CDROM)
	{
	...
	// should be 0x6C calculated above -> sizeof(STORAGE_DEVICE_DESCRIPTOR) + 0x24 + 9 + 17 + 5 + 1;
		...
		case SRB_FUNCTION_EXECUTE_SCSI:
		{
			...
			switch(Cdb->AsByte[0])
			{
				...
				case SCSIOP_MODE_SENSE:
				{
					...
					// return mode parameter header for HDD and CDROM
					else if(DeviceExtension->Disk.VirtualDeviceType == HDD || DeviceExtension->Disk.VirtualDeviceType == CDROM)
					{
